...
export function SearchSmall({ className }: SearchProps) {
  let [modifierKey, setModifierKey] = useState<string>()
  let { buttonProps, dialogProps } = useSearchProps()
  ...
  return (
    <>
      <button
        type="button"
        className={clsx(
          'inline-flex items-center gap-1 rounded-full bg-gray-950/2 px-2 py-1 inset-ring inset-ring-gray-950/8',
          className,
        )}
        {...buttonProps}
      >
        <SearchIcon className="size-5 flex-none fill-zinc-700 group-hover:fill-zinc-500 md:group-hover:fill-zinc-400" />
        <span className="text-zinc-700 md:sr-only md:ml-2">Search docs</span>
        {modifierKey && (
          <kbd className="ml-auto hidden font-medium text-zinc-700 md:block">
            <kbd className="font-sans">{modifierKey}</kbd>
            <kbd className="font-sans">K</kbd>
          </kbd>
        )}
